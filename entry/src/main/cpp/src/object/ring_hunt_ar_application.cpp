...
//   D (LOCKED) 进 10° / 出 15°(滞回)— 复用 kFrameAlignEnter/Exit
//   C/D → B(snapped 解锁)= 角度 > kAngleHudExitRad(20°)→ HUD 消失,炫彩框重现,允许重新吸附
constexpr float kFrameAlignEnterRad = 0.1745329f; // 10°
constexpr float kFrameAlignExitRad = 0.2617994f;  // 15°
constexpr float kAngleHudExitRad   = 0.3490659f;  // 20° — snapped 解锁阈值(C/D → B)
...
// 修(2026-06-26)— 放宽门限(0.75/0.70),优先保证用户能顺利触发 NGA 精调流程。
constexpr float kFillEnter = 0.75f;
constexpr float kFillExit  = 0.70f;
constexpr float kFillMax   = 1.00f;
constexpr float kFillEmaAlpha = 0.30f;
constexpr float kSnapHoldSec = 0.5f;   // snapReady 持续多久才触发吸附(特效 + 震动 + snappedOnce)
constexpr float kCGraceSec   = 0.5f;   // B→C 后,HUD 至少展示 0.5s 才允许 C→D
