...
namespace ArWorld {
    ...
    void WorldPlaneRenderer::InitializePlaneGlContent()
    {
        ...
        if (!LoadPngFromAssetManager(GL_TEXTURE_2D, "trigrid.png")) {
    ...
    void WorldPlaneRenderer::Draw(const glm::mat4 &projectionMat,
                                  const glm::mat4 &viewMat, const AREngine_ARSession *session,
    ...
    void WorldPlaneRenderer::UpdateForPlane(const AREngine_ARSession *session,
                                            const AREngine_ARPlane *plane)
    {
        vertices.clear();
        triangles.clear();
        ...
        normalVec = GetPlaneNormal(session, scopedArPose);
