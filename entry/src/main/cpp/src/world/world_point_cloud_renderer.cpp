...
namespace ArWorld {
    ...
    void WorldPointCloudRenderer::Draw(glm::mat4 mvpMatrix, const AREngine_ARSession *arSession,
        const AREngine_ARPointCloud *arPointCloud)
    {
        ...
        CHECK(HMS_AREngine_ARPointCloud_GetNumberOfPoints(arSession, arPointCloud, &numberOfPoints));
        if (numberOfPoints <= 0) {
            return;
        }

        const float *pointCloudData = nullptr;
        CHECK(HMS_AREngine_ARPointCloud_GetData(arSession, arPointCloud, &pointCloudData));
        if (pointCloudData) {
            pointClouldData.clear();
            pointClouldData.assign(pointCloudData, pointCloudData + numberOfPoints * 4);
