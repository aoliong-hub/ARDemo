...
{
    sprintf(mWorldFileManager.mRootPath, "%s", rootPath.c_str());
    LOGI("ARImageApp::SaveImageDataBaseToLocal mRootPath: %{public}s.", mWorldFileManager.mRootPath);
    if (strlen(mWorldFileManager.mRootPath) == 0) {
        LOGW("ARImageApp::SaveImageDataBaseToLocal rootPath is empty.");
        return;
    }

    if (mDataBase == nullptr) {
        LOGE("ARImageApp::SaveImageDataBaseToLocal dataBase is nullptr.");
        return;
    }
    mWorldFileManager.SaveImageDataBase(mArSession, mDataBase);
}

uint32_t ARImageApp::getImageCount() {
    
    if (mDataBase == nullptr) {
        LOGE("ARImageApp::getImageCount dataBase is nullptr.");
        return 0;
    }
    
    uint32_t imageCount = 0;
    HMS_AREngine_ARAugmentedImageDatabase_GetImageCount(mDataBase, &imageCount);
...
void ARImageApp::SetPath(const std::string rootPath)
{
    sprintf(mWorldFileManager.mRootPath, "%s", rootPath.c_str());
    LOGI("ARImageApp::SetPath() mRootPath: %{public}s.", mWorldFileManager.mRootPath);
    if (strlen(mWorldFileManager.mRootPath) == 0) {
        LOGW("ARImageApp::SetPath() rootPath is empty.");
        return;
    }
    ...
    mWorldFileManager.InitImageDataBaseFile();
    if (!mWorldFileManager.ReadImageDataBase(mDataBase)) {
        checkImage(mDataBase, "0001.jpg");
        checkImage(mDataBase, "100.png");
    }
}

int32_t ARImageApp::InitImage(size_t bufferLen, uint32_t width, uint32_t height, uint8_t *buffer)
{
    LOGI("ARImageApp::InitImage");
    std::lock_guard<std::mutex> lock(mImageMutex);
    if (mDataBase == nullptr) {
        LOGE("ARImageApp::InitImage mDataBase is nullptr.");
        return ARENGINE_ERROR_FATAL;
    }
    ImageBufferData data;
    data.height = height;
    data.width = width;
    data.buffer = buffer;
    data.bufferLen = bufferLen;
    int32_t err = checkImagePhoto(mDataBase, data);
    LOGW("The checkImagePhoto err is: %{public}d.", err);
